...
surprise the next time the screensaver runs. Review them through thumbnails instead
(`screensaver-shaders check`, below), which render offscreen.

## Add a pasted shader

1. Write the code verbatim to a file in your scratchpad, e.g. `incoming.frag`.
2. `screensaver-shaders check incoming.frag incoming.png` compiles it offscreen,
   renders a thumbnail at t=4s and prints brightness, motion and cost. Look at the
   thumbnail.
...
3. For each: `screensaver-shaders check <file> <file>.png`, look at the thumbnail and
   compare the stats with the quality bar. Iterate until it passes; a shader that is
   blown out, static, too slow or ugly gets fixed or replaced, not added.
...
For large batches it's fine to draft shaders in parallel subagents, but review every
thumbnail yourself before adding.
