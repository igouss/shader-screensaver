...
uniform float t;
uniform float f;
// twigl's sound level. There's no audio here, so it's never fed and reads 0.
uniform float s;
out vec4 o;
...
// twigl's noise helpers and constants, from its src/shader_snippet/noise.glsl.
// The simplex noise is Ian McEwan's (Ashima Arts, MIT License):
